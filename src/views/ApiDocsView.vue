...
import type { ApiEndpoint, ApiParam, ErrorCode } from '../types'

interface DocSection { id: string; label: string }
...
const BASE = 'https://api.mechai.io/v1'

function copyCode(id: string, text: string): void {
...
const params: ApiParam[] = [
  { f:'images',      t:'File[]',  r:true,  d:'1–8 images. JPG/PNG/WEBP. Max 20 MB each.'  },
...
const errorCodes: ErrorCode[] = [
  ...
  { code:413, label:'PAYLOAD_TOO_LARGE',   desc:'Image exceeds 20 MB limit'                               },
  ...
  { code:500, label:'INTERNAL_ERROR',      desc:'Something went wrong on our end. Retry after 30 s.'      },
]
    ...
    <div class="relative bg-bg-dark border-b border-orange/18 px-6 lg:px-[60px] py-8 overflow-hidden">
      ...
      <div class="relative z-10 flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          <code class="font-mono-tech text-[11px] text-cyan bg-bg-card px-3 py-1">{{ BASE }}</code>
        </div>
      </div>
    </div>
          ...
          <Transition name="fade" mode="out-in">
            ...
            <div v-if="activeSection === 'overview'" key="overview" class="flex flex-col gap-6">
              <MechCard padding="p-6">
                ...
                <p class="font-barlow-body text-[15px] text-cream-dim leading-[1.7] mb-5">
                  The MechAI REST API gives you programmatic access to our AI-powered vehicle
                  inspection engine. Upload photos and receive a structured defect report,
                  health score, and repair estimate in seconds.
                </p>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  class="bg-bg-card2 border border-orange/18 p-4 text-center">
                    ...
                    <div class="font-mono-tech text-[8px] uppercase tracking-[.15em] text-tan mt-1">
                      {{ stat.l }}
                    </div>
                  </div>
                </div>
                ...
                <div class="flex flex-col gap-0">
                  <div
                    ...
                    class="flex items-center gap-3 py-2.5 border-b border-white/5
                           hover:bg-white/2 transition-colors px-1"
                  >
                    <span
                      class="font-mono-tech text-[10px] font-bold border px-2 py-0.5
            ...
            <div v-else-if="activeSection === 'auth'" key="auth" class="flex flex-col gap-6">
              <MechCard padding="p-6">
                ...
                <p class="font-barlow-body text-[14px] text-cream-dim leading-[1.65] mb-5">
                  ...
                  <code class="text-cyan font-mono-tech text-[12px]">Bearer</code> token in the
                  <code class="text-cyan font-mono-tech text-[12px]">Authorization</code> header.
                  Generate API keys from
                  <RouterLink to="/settings" class="text-orange hover:text-amber-w transition-colors">
                    Settings → API Keys
                  </RouterLink>.
                </p>
                <CodeBlock id="auth_header" :code="snippets.auth_header" :copied="copied" @copy="copyCode" />
                <div class="mt-5 bg-amber-w/8 border border-amber-w/30 p-4 flex gap-3">
                  ...
                  <p class="font-barlow-body text-sm text-cream-dim">
                    Never expose API keys client-side. Always proxy requests through your backend.
                  </p>
                </div>
              </MechCard>
            </div>
            ...
            <div v-else-if="activeSection === 'scan'" key="scan" class="flex flex-col gap-6">
              <MechCard padding="p-6">
                ...
                <p class="font-barlow-body text-[14px] text-cream-dim leading-[1.65] mb-5">
                  Submit 1–8 vehicle images for AI analysis. Returns a
                  <code class="text-cyan text-[12px] font-mono-tech">scan_id</code> immediately.
                  Poll <code class="text-cyan text-[12px] font-mono-tech">GET /report/{scan_id}</code>
                  or use webhooks to receive results.
                </p>

                <div class="font-mono-tech text-[9px] tracking-[.18em] uppercase text-cyan mb-3">
                  Parameters
                </div>
                <table class="w-full border-collapse mb-5 text-sm">
                  <thead>
                    <tr class="border-b border-orange/18">
                      <th v-for="h in ['FIELD','TYPE','REQ','DESCRIPTION']" :key="h"
                          class="font-mono-tech text-[8px] text-tan text-left pb-2 pr-4">{{ h }}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="p in params" :key="p.f" class="border-b border-white/5">
                      ...
                      <td class="py-2.5 pr-4">
                        <span class="font-mono-tech text-[8px]"
                              :class="p.r ? 'text-hot-red' : 'text-tan'">
                ...
                <div class="font-mono-tech text-[9px] tracking-[.18em] uppercase text-cyan mb-3 mt-5">
                  JavaScript
                </div>
            ...
            <div v-else-if="activeSection === 'report'" key="report" class="flex flex-col gap-6">
              <MechCard padding="p-6">
                ...
                <p class="font-barlow-body text-[14px] text-cream-dim leading-[1.65] mb-5">
                  Returns the full inspection report once processing is complete.
                  Poll every 1–2 seconds until
                  <code class="text-cyan text-[12px] font-mono-tech">status === "complete"</code>.
                </p>
            ...
            <div v-else-if="activeSection === 'fleet'" key="fleet" class="flex flex-col gap-6">
              <MechCard padding="p-6">
                ...
                <div class="flex flex-col gap-3">
                  <div
                    v-for="ep in endpoints.filter(e => e.path.includes('fleet'))"
                    :key="ep.path"
            ...
            <div v-else-if="activeSection === 'webhooks'" key="webhooks" class="flex flex-col gap-6">
              <MechCard padding="p-6">
                ...
                <p class="font-barlow-body text-[14px] text-cream-dim leading-[1.65] mb-5">
                  Register an HTTPS endpoint to receive real-time scan results without polling.
                  MechAI signs every payload with
                  <code class="text-cyan text-[12px] font-mono-tech">X-MechAI-Signature</code>
                  (HMAC-SHA256).
                </p>
            ...
            <div v-else-if="activeSection === 'errors'" key="errors" class="flex flex-col gap-4">
              <MechCard padding="p-6">
                ...
                <div class="flex flex-col gap-0 divide-y divide-white/5">
                  <div
                    v-for="e in errorCodes" :key="e.code"
                    class="flex items-start gap-4 py-3.5 hover:bg-white/2 transition-colors"
                  >
                    <span
                      class="font-mono-tech text-[14px] font-bold w-12 flex-shrink-0"
