...
function load(): Vehicle[] {
  try {
    const raw = localStorage.getItem('mechai_fleet')
    return raw ? (JSON.parse(raw) as Vehicle[]) : SEED
  } catch {
    return SEED
  }
}
...
export const useFleetStore = defineStore('fleet', () => {
  const vehicles = ref<Vehicle[]>(load())

  const totalRepairCost = computed(() =>
    vehicles.value.reduce((s, v) => s + v.repairCost, 0)
  )
  const avgScore = computed(() =>
    Math.round(vehicles.value.reduce((s, v) => s + v.score, 0) / vehicles.value.length)
  )
  const criticalCount = computed(() =>
    vehicles.value.filter(v => v.status === 'crit').length
  )
  ...
  function save(): void {
    localStorage.setItem('mechai_fleet', JSON.stringify(vehicles.value))
  }

  function addVehicle(v: Omit<Vehicle, 'id'>): void {
    const id = 'F-' + String(vehicles.value.length + 1).padStart(3, '0')
    vehicles.value.unshift({ ...v, id })
    save()
  }
  ...
  function updateVehicle(id: string, patch: Partial<Vehicle>): void {
    ...
    if (i >= 0) {
      vehicles.value[i] = { ...vehicles.value[i], ...patch }
      save()
    }
  }

  return {
    vehicles, totalRepairCost, avgScore, criticalCount, byStatus,
