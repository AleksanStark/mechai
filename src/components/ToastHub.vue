...
<template>
  <Teleport to="body">
    <div class="fixed bottom-6 right-6 z-[9999] flex flex-col gap-3 pointer-events-none" style="max-width: 360px; width: calc(100vw - 48px)">
      <TransitionGroup name="toast">
        <div
          v-for="t in notif.toasts"
          :key="t.id"
          class="pointer-events-auto flex items-start gap-3 px-4 py-3.5
                 bg-bg-card border backdrop-blur-sm transition-all duration-300"
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="font-barlow-cond font-semibold text-cream text-[14px] leading-tight">
              {{ t.title }}
            </p>
            <p v-if="t.message" class="font-barlow-body text-xs text-cream-dim mt-0.5 leading-snug">
              {{ t.message }}
            </p>
          </div>
          ...
          <!-- Dismiss -->
          <button
            class="flex-shrink-0 font-mono-tech text-tan hover:text-cream text-xs
                   cursor-pointer bg-transparent border-0 mt-0.5"
